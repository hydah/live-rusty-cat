...
private:
    ...
    struct Argument {
        ...
        Argument(const String& _short_name, const String& _name, bool _optional, char nargs, const String& _desc)
                : short_name(_short_name), name(_name), optional(_optional), desc(_desc) {
            if (nargs == '+' || nargs == '*') {
...
public:
    ...
    void addArgument(const String& name, char nargs = 0, bool optional = true, const String& desc = "no desc") {
        if (name.size() > 2) {
            Argument arg("", verify(name), optional, nargs, desc);
            insertArgument(arg);
        } else {
            Argument arg(verify(name), "", optional, nargs, desc);
            insertArgument(arg);
    ...
    void addArgument(const String& short_name, const String& name, char nargs = 0,
                     bool optional = true, const String& desc = "no desc") {
        Argument arg(verify(short_name), verify(name), optional, nargs, desc);
        insertArgument(arg);
    }
    void addFinalArgument(const String& name, char nargs = 1, bool optional = false, const String& desc = "no desc") {
        final_name_ = delimit(name);
        Argument arg("", final_name_, optional, nargs, desc);
        insertArgument(arg);
    }
    ...
    String usage() {
        ...
        std::ostringstream d_help;
        help << "Usage: " << escape(app_name_);
        d_help << std::endl << std::endl;
        d_help << "Options:" << std::endl;
        ...
        for (ArgumentVector::const_iterator it = arguments_.begin(); it != arguments_.end(); ++it) {
            ...
            help << argstr;
            if (argstr.size() > 0) {
                d_help << '\t' << arg.realName() << '\t' << arg.toDesc() << std::endl;
            }
        }
        ...
        if (!final_name_.empty()) {
            ...
            help << argstr;
            if (argstr.size() > 0) {
                d_help << '\t' << argstr << '\t' << arg.toDesc() << std::endl;
            }
        }
        help << d_help.str();
    ...
    bool valid(const String& name) const { return used.count(delimit(name)) > 0; }
